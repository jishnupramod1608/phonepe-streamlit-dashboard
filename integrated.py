...
if page == "📂 Category Analysis":
    ...
    st.title("📂 FinanceWise Mentor – Category Analysis")
...
# =========================================================
elif page == "🔮 Expense and Savings Forecast":

    st.title("🔮 Expense and Savings Forecast (ARIMA)")

    # ✅ USE ALREADY UPLOADED FILE FROM SIDEBAR
    ...
    c1, c2, c3 = st.columns(3)
    c1.metric("Total Income", f"₹{monthly_income.sum():,.0f}")
    c2.metric("Total Expense", f"₹{monthly_expense.sum():,.0f}")
    c3.metric("Total Savings", f"₹{monthly_savings.sum():,.0f}")
    ...
    forecast_df = pd.DataFrame({
        ...
        "Predicted Income (₹)": income_fc.round(2),
        "Predicted Expense (₹)": expense_fc.round(2),
        "Predicted Savings (₹)": savings_fc.round(2)
    })
